...
A Lei de Amdahl~\cite{amdahl1967} delimita o ganho máximo de qualquer
otimização paralela. Sendo $P$ a fração paralelizável do trabalho e
$N$ o número de unidades de execução:
...
25\,000 a 70\,000 erros corrigíveis por \textsc{dimm} ao ano e
mostrou que aproximadamente 8\,\% dos \textsc{dimm}s exibem ao menos
um erro por ano --- contrariando a hipótese anterior de erros raros
ou esporádicos.

A correção em tempo real é feita por um código corretor proposto
originalmente por Hamming~\cite{hamming1950} em 1950. A variante
\textsc{secded} (\emph{Single Error Correction, Double Error
Detection}), padrão em \textsc{dimm} \textsc{ecc} registrado, anexa
quatro bits de paridade nas posições $2^k$ ($k=0,1,2,3$) e um bit
adicional global. Cada bit de paridade cobre um subconjunto de
posições determinado por seu bit correspondente em binário; na
leitura, o controlador recalcula as paridades e obtém um \emph{síndrome}
cuja representação decimal é exatamente a posição do bit corrompido.
O hardware inverte o bit indicado e entrega o dado correto à CPU sem
qualquer participação do sistema operacional.

A norma JEDEC \textsc{jesd79-5}~\cite{jedec2024ddr5} tornou
...
\textit{host} --- isolamento é obtido por \textit{namespaces}
(PID, mount, network, IPC, UTS, user) e \textit{cgroups} do
Linux~\cite{bovet2005}. O resultado: \textit{boot} em $\approx
...
O \emph{Completely Fair Scheduler} (\textsc{cfs}) do Linux mantém
todas as \texttt{task\_struct} numa árvore rubro-negra ordenada por
\texttt{vruntime} --- tempo virtual de execução ponderado pela
prioridade \emph{nice}. A cada decisão, escolhe-se a tarefa mais à
esquerda; complexidade $\mathcal{O}(\log n)$~\cite{bovet2005}. O
